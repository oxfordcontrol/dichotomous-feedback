...
for ii = 1 : length(input_ompr)
    for jj = 1 : length(input_omprc)
        ...
        [t_hill2, x_hill2] = ode15s(@(t,x)two_comp_sink_hill_ol(t,x,params),[0,T],x0_rr, ops);
        omprp_ol(ii,jj,:)  = x_hill2(end,[3*n_t + 1 : 4*n_t]);
        omprcp_ol(ii,jj,:) = x_hill2(end,[5*n_t + 1 : 6*n_t]);
        x_prot_ol(ii,jj,:) = x_hill2(end,[6*n_t + 1 : 7*n_t]);
    end
end
...
for ii = 1 : length(input_ompr)
    for jj = 1 : length(input_p)
        ...
        x_prot_cl(ii,jj,:) = x_rr_cl{ii,jj}(end,[6*n_t + 1 : 7*n_t]);
    end
end
