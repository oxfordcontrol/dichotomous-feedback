...
syms Taz TazP TazC OmpRP OmpRsum OmpRC OmpRCsum kap P tlat_ompr beta_r Asp

num   = (OmpRP./params.Kdr).^params.hill_coeff; %  params.tx_gfp*OmpRP;%
tl_x  = (params.tx_gfp*(num./(num + 1)));
tl_rc  = P*params.tx_gfp*num./(num + 1);
fp_rr  = [params.tlat_Taz-params.delta*Taz-kap*Taz+(TazP)*(params.kt*(OmpRsum - OmpRP)+params.ktc*OmpRC); ...
	  -params.delta*TazP+kap*Taz-(TazP)*(params.kt*(OmpRsum - OmpRP)+params.ktc*OmpRC);...
      -params.delta*OmpRP+params.kt*(TazP)*(OmpRsum - OmpRP)-params.kp*Taz.*OmpRP;...
      beta_r - params.delta*OmpRsum]; 
fk_rr  = [tl_rc-params.delta*OmpRC-params.ktc*(TazP)*OmpRC+params.kpc*Taz*(OmpRCsum - OmpRC);... 
...
for ii = 1 : length(input_ompr)
    for jj = 1 : length(input_p)
        for kk = 1 : length(params.Atot)
            ...
            Bprr_val = eval(subs(Bp_rr, [Taz; TazP], ...
