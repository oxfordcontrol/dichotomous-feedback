...
asp_grid = 2;%[.5, 1, 2, 5, 10]; %[logspace(-1, log10(2),8), linspace(2.1, 10, 7)];
...
input_ompr = 6;
input_p    = 1;%[0.1, 0.4, 0.7, 1];%linspace(0, 1, n_trc);
% w = [1e-4, 1e-3, 1e-2, 3e-2, 5e-2, 7e-2, 1e-1,  5e-1]/2/pi;
w = [1e-4, 1e-3, 1e-2, 1e-1, 2e-1, 23e-2, 25e-2, 27e-2, 3e-1, 5e-1, 1]/2/pi;
T  = 1e+4;
...
x_all = zeros(length(input_ompr), length(input_p), length(x0_rr));
x_prot_smallsignal = cell(length(input_ompr), length(input_p), length(w));
t_smallsignal = cell(length(input_ompr), length(input_p), length(w));
...
for ii = 1 : length(input_ompr)
    for jj = 1 : length(input_p)
        ...
        parfor kk = 1:length(w)
            [t_smallsignal{ii, jj, kk}, x_smallsignal] = ode15s(@(t,x)two_comp_sink_small_signal(t,x,params, w(kk)),[0,1e+6], x_hill(end,:), ops);
            x_prot_smallsignal{ii, jj, kk} = x_smallsignal(:,[3*n_t + 1 : 4*n_t]);
        end
    end
end
...
hold on
for kk = 1:length(w) 
    t_ind = find(t_smallsignal{1,1,kk} >= 1000, 1, 'first');
    mag_(kk) = (max((x_prot_smallsignal{1, 1, kk}(t_ind:end,:)))-min(x_prot_smallsignal{1, 1, kk}(t_ind:end,:)))/1e-6/2;
%     ()-mean(x_prot_smallsignal{1, 1, kk}))/1e-3
    plot(t_smallsignal{1,1,kk}(1:end), x_prot_smallsignal{1,1,kk}(1:end,:))
    
end
% plot(omprp_rr_ss(1,1,:))
return
syms Taz Tazsum TazC OmpRP OmpRsum OmpRC OmpRCsum kap P tlat_ompr beta_r Asp

num   = (OmpRP./params.Kdr).^params.hill_coeff; %  params.tx_gfp*OmpRP;%
tl_x  = (params.tx_gfp*(num./(num + 1)));
tl_rc  = P*params.tx_gfp*num./(num + 1);
fp_rr  = [params.tlat_Taz-params.delta*Taz-kap*Taz+(Tazsum - Taz)*(params.kt*(OmpRsum - OmpRP)+params.ktc*OmpRC); ...
      -params.delta*OmpRP+params.kt*(Tazsum - Taz)*(OmpRsum - OmpRP)-params.kp*Taz.*OmpRP;...
      beta_r - params.delta*OmpRsum]; 
fk_rr  = [tl_rc-params.delta*OmpRC-params.ktc*(Tazsum - Taz)*OmpRC+params.kpc*Taz*(OmpRCsum - OmpRC);... 
       tl_rc-params.delta*OmpRCsum];
  
Ap_rr = jacobian(fp_rr, [Taz; OmpRP; OmpRsum]);
Bp_rr = jacobian(fp_rr, [OmpRC; beta_r]);
Bprr_a = [-k_a_max*(Asp)*Taz/(K_d_a+(Asp)); 0; 0]
Cp_rr = [1 0 0;0 1 0];

Ak_rr = jacobian(fk_rr, [OmpRC; OmpRCsum]);
Bk_rr = jacobian(fk_rr, [Taz; OmpRP]);
Ck_rr = [1, 0];
...
for ii = 1 : length(input_ompr)
    for jj = 1 : length(input_p)
        for kk = 1 : length(params.Atot)
            Aprr_val = eval(subs(Ap_rr, [Taz; OmpRP; Tazsum; OmpRsum; OmpRC; kap; P], ...
            ...
            sens_fun_rr(ii,jj,kk)    = norm(inv(eye(1)-K_rr{ii,jj,kk}*G1_rr{ii,jj,kk}),'inf');
...
for ii = 1 :1 % length(input_ompr)
    for jj = 1 :  length(input_p)
        for kk2 = 3:3%length(k_grid):length(k_grid)
            kk = k_grid(kk2);
            [sv,~] = sigma(inv(eye(1)-K_rr{ii,jj,kk}*G1_rr{ii,jj,kk}),w);
