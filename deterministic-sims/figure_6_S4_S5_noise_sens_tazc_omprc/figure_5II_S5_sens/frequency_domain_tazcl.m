...
params.kt =6.12;%4.15;
params.kp = 0.00294*60*1e-2;%1.37; 
...
tazsum_tc_ss  = zeros(length(input_ompr), length(input_p), n_t);
ompr_tc_ss    = zeros(length(input_ompr), length(input_p), n_t);
omprp_tc_ss   = zeros(length(input_ompr), length(input_p), n_t);
omprsum_tc_ss = zeros(length(input_ompr), length(input_p), n_t);
...
syms Taz Tazsum OmpRP OmpRsum TazC kap P tlat_ompr beta_r Asp

num   = (OmpRP./params.Kdr).^params.hill_coeff; %  params.tx_gfp*OmpRP;%
tl_x  = (params.tx_gfp*(num./(num + 1)));
tl_t  = P*params.tx_gfp*num./(num + 1);
fp_tc  = [params.tlat_Taz-params.delta*Taz-kap*Taz+(Tazsum - Taz)*(params.kt*(OmpRsum - OmpRP)); ...
      -params.delta*OmpRP+params.kt*(Tazsum - Taz)*(OmpRsum - OmpRP)-(params.kp*Taz +params.kpc*TazC).*OmpRP;...
...
sens_fun_tc     = zeros(length(input_ompr), length(input_p), n_t);
r_resp_tc       = zeros(length(input_ompr), length(input_p), n_t);
G1_tc = cell(length(input_ompr), length(input_p), n_t);
G2_tc = cell(length(input_ompr), length(input_p), n_t);
K_tc = cell(length(input_ompr), length(input_p), n_t);

for ii = 1 : length(input_ompr)
    for jj = 1 : length(input_p)
        for kk = 1 : length(params.Atot)
            Aptc_val = eval(subs(Ap_tc, [Taz; OmpRP; Tazsum; OmpRsum; TazC; kap; P], ...
                                   [taz_tc_ss(ii,jj,kk); omprp_tc_ss(ii,jj,kk); ...
                                    tazsum_tc_ss(ii,jj,kk); omprsum_tc_ss(ii,jj,kk); ...
                                    tazc_tc_ss(ii,jj,kk); params.kap_taz(kk); input_p(jj)]));
            Bptc_val = eval(subs(Bp_tc, [OmpRP], ...
                                   [omprp_tc_ss(ii,jj,kk)]));
            Bptca_val = eval(subs(Bptc_a, [Taz; Asp], ...
                                   [taz_tc_ss(ii,jj,kk); params.Atot(kk)]));
            Bktc_val = eval(subs(Bk_tc, [OmpRP; P], ...
                                   [omprp_tc_ss(ii,jj,kk); input_p(jj)]));
            ...
            sens_fun_tc(ii,jj,kk) = norm(inv(eye(1)-G1_tc{ii,jj,kk}*K_tc{ii,jj,kk}),'inf');
