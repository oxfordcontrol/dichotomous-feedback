...
for ii = 1 : length(input_ompr)
    for jj = 1 : length(input_p)
        ...
        params.P = input_p(jj);        
        [t_hill, x_hill] = ode15s(@(t,x)two_comp_taz_tx(t,x,params),[0,T],x0_tc, ops);
...
syms Taz Tazsum TazC OmpRP OmpRsum FC kap P tlat_ompr beta_r Asp

num   = (OmpRP./params.Kdr).^params.hill_coeff; %  params.tx_gfp*OmpRP;%
tl_x  = (params.tx_gfp*(num./(num + 1)));
tl_rc  = P*params.tx_gfp*num./(num + 1);
beta_t = params.tlat_Taz./(1 + (FC./params.Kdfc).^1);
fp_rr  = [beta_t - params.delta*Taz-kap*Taz+(Tazsum - Taz)*(params.kt*(OmpRsum - OmpRP)); ...
          beta_t - params.delta*Tazsum; ...
          -params.delta*OmpRP+params.kt*(Tazsum - Taz)*(OmpRsum - OmpRP)-params.kp*Taz.*OmpRP;...
          beta_r - params.delta*OmpRsum]; 
fk_rr  = [tl_rc-params.delta*FC];
...
for ii = 1 : length(input_ompr)
    for jj = 1 : length(input_p)
        for kk = 1 : length(params.Atot)
            Aprr_val = eval(subs(Ap_rr, [Taz; OmpRP; Tazsum; OmpRsum; FC; kap; P], ...
            ...
            Bprr_val = eval(subs(Bp_rr, [OmpRP; Taz; Tazsum; FC], ...
                                   [omprp_rr_ss(ii,jj,kk); taz_rr_ss(ii,jj,kk); tazsum_rr_ss(ii,jj,kk); fc_rr_ss(ii,jj,kk)]));
            Bprra_val = eval(subs(Bprr_a, [Taz; Asp], ...
                                   [taz_rr_ss(ii,jj,kk); params.Atot(kk)]));
            Akrr_val = eval(subs(Ak_rr, [OmpRP; Taz; Tazsum], ...
                                   [omprp_rr_ss(ii,jj,kk);taz_rr_ss(ii,jj,kk); tazsum_rr_ss(ii,jj,kk)]));
