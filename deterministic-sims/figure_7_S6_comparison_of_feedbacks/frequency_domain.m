...
params.delta_Taz    = d_protein; %0.03;
...
for ii = 1 : length(input_ompr)
    for jj = 1 : length(input_p)
        for kk = 1 : length(params.Atot)
            ...
            sens_fun_rr(ii,jj,kk)    = norm(inv(eye(1)-K_rr{ii,jj,kk}(2)*G1_rr{ii,jj,kk}(2)),'inf');
...
for ii = 1 :1 % length(input_ompr)
    for jj = 1 :  length(input_p)
        for kk2 = 3:3%length(k_grid):length(k_grid)
            kk = k_grid(kk2);
            [sv,~] = sigma(inv(eye(1)-G1_rr{ii,jj,kk}(2)*K_rr{ii,jj,kk}(2)),w);
