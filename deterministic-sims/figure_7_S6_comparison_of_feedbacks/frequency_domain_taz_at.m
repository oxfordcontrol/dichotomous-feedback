...
ops = odeset('AbsTol', 1e-30, 'RelTol', 1e-13);
...
for ii = 1 : length(input_ompr)
    for jj = 1 : length(input_p)
        ...
        params.P = input_p(jj);        
        [t_hill, x_hill] = ode15s(@(t,x)two_comp_taz_at(t,x,params),[0,T],x0_tc, ops);
...
syms Taz TazP OmpRP OmpR OmpRsum FC kap P tlat_ompr beta_r Asp

num   = (OmpRP./params.Kdr).^params.hill_coeff; %  params.tx_gfp*OmpRP;%
tl_x  = (params.tx_gfp*(num./(num + 1)));
tl_rc  = P*params.tx_gfp*num./(num + 1);
fp_rr  = [params.tlat_Taz-params.delta*Taz-kap*Taz+params.kt*TazP*OmpR-params.kpc*FC*Taz; ...
	-params.delta*TazP+kap*Taz-params.kt*TazP*OmpR-params.kpc*FC*TazP;...
        -params.delta*OmpRP+params.kt*TazP*OmpR-params.kp*Taz*OmpRP;...
         beta_r-params.delta*OmpR-params.kt*TazP*OmpR+params.kp*Taz*OmpRP]; 
fk_rr  = [tl_rc-params.delta*FC-params.kpc*FC*Taz-params.kpc*FC*TazP];
  
Ap_rr = jacobian(fp_rr, [Taz; TazP; OmpRP; OmpR]);
...
Bk_rr = jacobian(fk_rr, [OmpRP; Taz; TazP]);
Ck_rr = [1];

Acl = jacobian([fp_rr; fk_rr], [Taz; TazP; OmpRP; OmpR; FC]);
...
for ii = 1 : length(input_ompr)
    for jj = 1 : length(input_p)
        for kk = 1 : length(params.Atot)
            Aprr_val = eval(subs(Ap_rr, [Taz; OmpRP; TazP; OmpR; FC; kap; P], ...
                                   [taz_rr_ss(ii,jj,kk); omprp_rr_ss(ii,jj,kk); ...
                                    tazp_rr_ss(ii,jj,kk); ompr_rr_ss(ii,jj,kk); ...
                                    fc_rr_ss(ii,jj,kk); params.kap_taz(kk); input_p(jj)]));
            Bprr_val = eval(subs(Bp_rr, [OmpRP; TazP; Taz], ...
                                   [omprp_rr_ss(ii,jj,kk); tazp_rr_ss(ii,jj,kk); taz_rr_ss(ii,jj,kk)]));
            Bprra_val = eval(subs(Bprr_a, [Taz; Asp], ...
                                   [taz_rr_ss(ii,jj,kk); params.Atot(kk)]));
            Akrr_val = eval(subs(Ak_rr, [OmpRP; TazP; Taz], ...
                                   [omprp_rr_ss(ii,jj,kk);tazp_rr_ss(ii,jj,kk); taz_rr_ss(ii,jj,kk)]));
            Bkrr_val = eval(subs(Bk_rr, [FC; OmpRP; P], ...
                                   [fc_rr_ss(ii,jj,kk); ...
                                    omprp_rr_ss(ii,jj,kk); input_p(jj)]));
	    Acl_val = eval(subs(Acl, [Taz; TazP; OmpRP; OmpR; FC; kap; P], ...
   				   [taz_rr_ss(ii,jj,kk); tazp_rr_ss(ii,jj,kk);...
                                    omprp_rr_ss(ii,jj,kk); ompr_rr_ss(ii,jj,kk); ...
                                    fc_rr_ss(ii,jj,kk); params.kap_taz(kk); input_p(jj)]));
	    [ii,jj,kk, max(real(eig(Acl_val)))]
...
for ii = 1 :1 % length(input_ompr)
    for jj = length(input_p) :  length(input_p)
        for kk2 = 1:length(k_grid)
            kk = k_grid(kk2);
            [sv,~] = sigma([1 0 0]*inv(eye(3)-G1_rr{ii,jj,kk}*K_rr{ii,jj,kk})*G3_rr{ii,jj,kk}/d_protein,w);
